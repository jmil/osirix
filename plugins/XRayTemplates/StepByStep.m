...
- (IBAction)nextStep:(id)sender;
{
	...
	else
	{
		...
		if([[steps objectAtIndex:currentStepIndex] isDone])
		{
			[self nextStep:nil];
			return;
		}
...
//	[view setStepAtIndex:0 enabled:YES];
...
- (BOOL)souldReset;
{
	BOOL souldReset = NO;
	int i;	
	for(i=0; i<[steps count] && !souldReset; i++)
	{
		souldReset = [[steps objectAtIndex:i] isDone];
	}
	return souldReset;
}

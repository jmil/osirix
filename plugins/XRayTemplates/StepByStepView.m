...
{
	NSLog(@"adjustWindow");
	float upY = [[self window] frame].origin.y + [[self window] frame].size.height;
	
	NSRect newWindowFrame = [[self window] frame];
	//newWindowFrame.size.height = [self frame].size.height+60;

	float heightShift = [self frame].size.height - oldFrame.size.height;
	
	oldFrame = [self frame];
	
	newWindowFrame.size.height += heightShift;
	
	newWindowFrame.origin.y = upY - newWindowFrame.size.height;
...
- (void)stepViewDidExpandNotification:(NSNotification*)notification;
{
	...
	[controller enableSteps];
	NSLog(@"stepViewDidExpandNotification");
	[self computeStepViewFrames];
}

- (void)stepViewDidCollapseNotification:(NSNotification*)notification;
{
	NSLog(@"stepViewDidCollapseNotification");
	[self computeStepViewFrames];
}

- (void)stepViewDidToggleNotification:(NSNotification*)notification;
{
	NSLog(@"stepViewDidToggleNotification");
	[self adjustWindow];
}

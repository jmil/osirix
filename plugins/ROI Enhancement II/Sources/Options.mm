...
//	NSLog(@"Axes %@", [[[_interface chart] axes] properties]);
//	NSLog(@"Chart %@", [[_interface chart] properties]);
	[self yRangeChanged: NULL]; // update the displayed numeric range
}

-(IBAction)curvesChanged:(id)sender {
	[[_interface roiList] changedMin:[_minCurve state] mean:[_meanCurve state] max:[_maxCurve state]];
